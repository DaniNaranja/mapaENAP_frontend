    ...
    <div v-if="isVisible" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
        ...
        <div class="flex-1 pr-4">
          ...
          <form @submit.prevent="submitForm">
            ...
            <div class="mb-4">
              ...
              <input type="date" v-model="form.fecha" id="fecha" class="w-full p-2 border rounded bg-gray-200" required />
            </div>
  
            <!-- Solicitante -->
            <div class="mb-4">
              <label for="solicitante" class="block text-sm font-semibold mb-1">Solicitante</label>
              <input type="text" v-model="form.solicitante" id="solicitante" class="w-full p-2 border rounded bg-gray-200" required />
            </div>
  
            <!-- Motivo -->
            <div class="mb-4">
              <label for="motivo" class="block text-sm font-semibold mb-1">Motivo</label>
              <textarea v-model="form.motivo" id="motivo" class="w-full p-2 border rounded bg-gray-200"></textarea>
            </div>
  
            <!-- Fechas de inicio y termino -->
            <div class="mb-4">
              <label for="inicio" class="block text-sm font-semibold mb-1">Inicio</label>
              <input type="datetime-local" v-model="form.inicio" id="inicio" class="w-full p-2 border rounded bg-gray-200" required />
            </div>
  
            <div class="mb-4">
              <label for="termino" class="block text-sm font-semibold mb-1">Término</label>
              <input type="datetime-local" v-model="form.termino" id="termino" class="w-full p-2 border rounded bg-gray-200" required />
            </div>
  
            <!-- Calle -->
            <div class="mb-4">
              <label for="calle" class="block text-sm font-semibold mb-1">Calle</label>
              <input type="text" v-model="form.calle" id="calle" class="w-full p-2 border rounded bg-gray-200" required />
            </div>
            ...
            <div class="mb-4">
              ...
              <div class="flex">
                <input type="text" v-model="form.latitud" placeholder="Latitud" class="w-1/2 p-2 bg-gray-200 border rounded mr-2" readonly />
                <input type="text" v-model="form.longitud" placeholder="Longitud" class="w-1/2 p-2 bg-gray-200 border rounded" readonly />
              </div>
            </div>
            ...
            <div class="mb-4">
              ...
              <textarea v-model="form.observacion" id="observacion" class="w-full p-2 border rounded bg-gray-200"></textarea>
            </div>
      ...
      <div class="w-1/3 relative">
        ...
        <div class="absolute bottom-4 right-4 flex space-x-2">
          <button type="button" @click="close" class=" px-7 py-3 bg-gray-500 text-lg text-white font-bold rounded hover:bg-gray-700 mr-3">Cancelar</button>
          <button type="submit" class="px-10 py-3 bg-green-500 text-lg text-white font-bold rounded hover:bg-green-700">Enviar</button>
        </div>
      </div>
      </div>
    </div>
