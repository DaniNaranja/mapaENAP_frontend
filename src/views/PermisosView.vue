    ...
    <div class="space-y-4">
      ...
      <div v-for="permiso in permisos" :key="permiso.id" @click="openModal(permiso)"
        ...
        <div class="grid grid-cols-6 gap-4 mt-2">
          ...
          <div class="text-base text-gray-800 capitalize">{{ permiso.tipo }}</div>
          <div class="text-base text-gray-800">{{ permiso.solicitante }}</div>
          <div class="text-base text-gray-800">{{ permiso.motivo }}</div>
...
<div v-if="isModalVisible" class="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center z-50">
  <div class="bg-white p-6 rounded-lg shadow-lg w-3/4 flex h-[90vh] relative">
    ...
    <button
      @click="closeModal"
      class="absolute top-4 right-4 text-2xl font-bold text-gray-600 hover:text-gray-800"
    >
      <i class="fa-solid fa-xmark"></i>
    </button>

    <!-- Contenedor de texto -->
    <div class="w-2/3 pr-4 mb-3">
    ...
    <!-- Contenedor del mapa -->
    <div class="w-1/2 h-full rounded-xl border flex flex-col mt-6">
      <!-- Mapa -->
      <div id="map" class="h-5/6 w-full rounded-xl"></div>

      <!-- Botones de Autorizar y Rechazar -->
      <div class="flex justify-between mt-10">
        <button @click="autorizarPermiso" class="bg-green-500 text-white p-4 rounded-lg w-1/2 mr-2 hover:bg-green-600">
          Autorizar
        </button>
